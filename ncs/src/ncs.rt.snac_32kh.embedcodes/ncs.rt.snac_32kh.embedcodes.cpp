// -----------------------------------------------------------------------------
// ncs.rt.snac_32kh.embedcodes  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 32kHz codebook lookup, real-time variant: identical math to
// ncs.snac_32kh.embedcodes (same decode_codes.onnx graph -- the codebook
...
// Identical network to snac_44kh (only sampling rate and weights
...
// ncs.rt.snac_32kh.decode~'s running context -- every block must be
// looked up in order, none can be dropped. So (like ncs.rt.snac_32kh.vq)
...
// ncs.rt.snac_32kh.encode_tilde.cpp's kBlockSize(12288)/kModelSampleRate(32000);
...
static constexpr double kBlockDurationMs = 12288.0 / 32000.0 * 1000.0;
...
// ncs.rt.snac_32kh.encode_tilde.cpp's kMaxChunkAtoms comment: this is a
...
class NcsRtSnac_32khEmbedcodes : public object<NcsRtSnac_32khEmbedcodes>
{
public:
    MIN_DESCRIPTION     {"SNAC 32kHz codebook lookup (real-time): per-level codes block -> summed + per-level quantized embeddings."};
    MIN_TAGS            {"snac, onnx"};
    ...
    // This matches ncs.rt.snac_32kh.vq, whose outlets fire right-to-left
    // (level3 first, level0 last), so a direct patch cord hookup lands
    // the cold values before the hot trigger arrives.
    inlet<> level0_in{ this, "(list/load) codes... from ncs.rt.snac_32kh.vq level0 -- hot, or model path" };
    inlet<> level1_in{ this, "(list) codes... from ncs.rt.snac_32kh.vq level1 -- cold" };
    inlet<> level2_in{ this, "(list) codes... from ncs.rt.snac_32kh.vq level2 -- cold" };
    inlet<> level3_in{ this, "(list) codes... from ncs.rt.snac_32kh.vq level3 -- cold" };

    // Leftmost outlet is always the sum (each level scaled by its
    // level<N>_scale before summing) -- this is the one that normally
    // feeds ncs.rt.snac_32kh.decode~; the rest are always the raw,
    ...
    // Parses ncs.rt.snac_32kh.vq's format: a bare (headerless) codes list
            ...
            try {
                ...
                if (inlet == 1) {
                    ...
                    if (cached_level1_.empty() || cached_level2_.empty() || cached_level3_.empty()) {
                        log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: level1/level2/level3 codes not yet received "
                              "— connect all four of ncs.rt.snac_32kh.vq's outlets to inlets 0-3"});
                        return {};
                    }
                    EmbedRequest req;
                ...
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: list handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EmbedRequest& req) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: no model loaded"});
            return;
        }
        auto result = run_onnx(req);
        if (result.empty() || result.size() != kNumLevels + 1) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: ONNX Runtime not available"});
                return;
            }
            // 4 intra-op threads for consistency with encode~/decode~
            // (see ncs.rt.snac_32kh.encode_tilde.cpp's comment) -- this
            ...
            if (input_names_.size() != kNumLevels || output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac_32khEmbedcodes);
