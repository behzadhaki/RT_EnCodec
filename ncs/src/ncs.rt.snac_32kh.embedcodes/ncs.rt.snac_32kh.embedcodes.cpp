...
public:
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                error("ncs.rt.snac_32kh.embedcodes: load requires a file path");
                return {};
            }
...
private:
    ...
    void load_model(const std::string& path) {
        try {
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.rt.snac_32kh.embedcodes: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.embedcodes: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
