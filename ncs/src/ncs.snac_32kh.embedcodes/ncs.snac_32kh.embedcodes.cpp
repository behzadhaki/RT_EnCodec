// -----------------------------------------------------------------------------
// ncs.snac_32kh.embedcodes  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// native temporal resolution -- matches ncs.snac_32kh.vq's 4 outlets
...
// summing. Identical network to snac_44kh (only sampling rate and
...
// Codes in (from ncs.snac_32kh.vq, per level) are a single bare
// (headerless) list -- vq never chunks its output, so no frame/last
// protocol is needed there. Embeddings out (to ncs.snac_32kh.decode and
// friends, per level, chunked) mirror ncs.snac_32kh.encode's data_out
...
// ncs.snac_32kh.decode gets it directly from ncs.snac_32kh.encode's
...
class NcsSnac_32khEmbedcodes : public object<NcsSnac_32khEmbedcodes>, public CancellableRun
{
public:
    ...
    // This matches ncs.snac_32kh.vq, whose outlets fire right-to-left
    // (level3 first, level0 last), so a direct patch cord hookup lands
    // the cold values before the hot trigger arrives.
    inlet<> level0_in{ this, "(list/load) codes... from ncs.snac_32kh.vq level0 -- hot, or model path" };
    inlet<> level1_in{ this, "(list) codes... from ncs.snac_32kh.vq level1 -- cold" };
    inlet<> level2_in{ this, "(list) codes... from ncs.snac_32kh.vq level2 -- cold" };
    inlet<> level3_in{ this, "(list) codes... from ncs.snac_32kh.vq level3 -- cold" };
    ...
    // stream, same format as ncs.snac_32kh.encode's data_out.
    ...
    // Parses ncs.snac_32kh.vq's format: a bare (headerless) codes list per
            ...
            try {
                ...
                if (inlet == 1) {
                    ...
                    if (cached_level1_.empty() || cached_level2_.empty() || cached_level3_.empty()) {
                        log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: level1/level2/level3 codes not yet received "
                              "— connect all four of ncs.snac_32kh.vq's outlets to inlets 0-3"});
                        return {};
                    }
                    EmbedRequest req;
                    ...
                    // still doing -- see ncs.snac_32kh.encode's bang_msg
                    // comment for why this cancels rather than waits.
                    cancel_active_run();
                    input_queue_.enqueue(std::move(req));
                }
            } catch (const std::exception& ex) {
                log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: list handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EmbedRequest& req) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: no model loaded"});
            return;
        }
        auto result = run_onnx(req);
        if (result.empty() || result.size() != kNumLevels + 1) {
            log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: ONNX Runtime not available"});
                return;
            }
            ...
            if (input_names_.size() != kNumLevels || output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: expected " + std::to_string(kNumLevels)
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.snac_32kh.embedcodes: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.snac_32kh.embedcodes: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
...
MIN_EXTERNAL(NcsSnac_32khEmbedcodes);
