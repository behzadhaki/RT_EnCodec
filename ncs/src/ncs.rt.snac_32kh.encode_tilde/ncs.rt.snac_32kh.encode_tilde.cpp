// -----------------------------------------------------------------------------
// ncs.rt.snac_32kh.encode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// matching ncs.snac_32kh.encode's padding rule) so every block's frame
...
// and ncs.rt.snac_32kh.vq reassembles it -- unlike snac_24kh.rt's plain
// "list" transport.
//
...
// cost on ncs.rt.snac_32kh.embedcodes -> ncs.rt.snac_32kh.decode~'s hop
...
class NcsRtSnac_32khEncode : public object<NcsRtSnac_32khEncode>
...
public:
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: no model loaded"});
            return;
        }
        auto z = run_onnx(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.encode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac_32khEncode);
