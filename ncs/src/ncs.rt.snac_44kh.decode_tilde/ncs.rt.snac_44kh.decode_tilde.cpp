// -----------------------------------------------------------------------------
// ncs.rt.snac_44kh.decode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 44kHz decoder, real-time variant. Mirrors ncs.rt.snac_44kh.encode~'s
// windowed context-trim approach (see that file's class comment, and
// ncs.rt.snac_24kh.decode_tilde.cpp for the full rationale: no
...
//    ncs.rt.snac_44kh.embedcodes), not a signal -- so the context
...
class NcsRtSnac_44khDecode : public object<NcsRtSnac_44khDecode>
...
public:
    ...
    inlet<> embeddings_in{ this, "(messageix/tensor/load/reset) quantized embeddings chunk from ncs.rt.snac_44kh.embedcodes, channel-major [1024 x T], or model path" };
    ...
    sample operator()() {
        ...
        if (!was_underrun_) {
            was_underrun_ = true;
            log_underrun_transition(true);
        }
        return 0.0;
    }

    // ncs.rt.snac_44kh.embedcodes sends its sum_out embeddings as a
    ...
    // snac_24kh's rt~ plain "list") because a 32k/44k block (32768
    // atoms) is well past the single-list-message crash threshold
    // documented in ncs.snac_24kh.encode.cpp. Once the last chunk
        ...
        MIN_FUNCTION {
            try {
                ...
                if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(new_zq.size())) {
                    log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: embeddings block size is not a multiple of 1024 channels"});
                ...
                log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: tensor handling failed — " + std::string(ex.what())});
                accum_embeddings_.clear();
                pending_is_last_ = false;
            }
            return {};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    std::chrono::steady_clock::time_point last_underrun_log_{};
    ...
    void process(const DecodeJob& job) {
        ...
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: unexpected output length"});
            return;
        }
    ...
    // Rate-limits underrun/recovery logging to at most one message per
    // second, regardless of how many times the state actually flips
    // within that window. 32k/44k run close enough to the edge of
    // real-time that underrun/recovery can oscillate many times per
    // second rather than as one long dropout -- logging every single
    // transition means flush_output()'s post() calls (on the SAME
    // 10ms main-thread timer that also relays the actual chunked
    // embeddings data between encode~/vq/embedcodes/decode~) can
    // themselves start competing for main-thread time with the data
    // relay, making a borderline-real-time pipeline worse instead of
    // just reporting it. Called from the audio thread (under
    // audio_mutex_, like the rest of operator()); std::chrono is safe
    // to use there (no allocation/locking beyond what log_queue_.enqueue
    // already does on a state change).
    void log_underrun_transition(bool entering_underrun) {
        auto now = std::chrono::steady_clock::now();
        if (now - last_underrun_log_ < std::chrono::milliseconds(1000))
            return;
        last_underrun_log_ = now;
        if (entering_underrun)
            log_queue_.enqueue({true, "ncs.rt.snac_44kh.decode~: underrun — outputting silence (worker/model too slow, or nothing decoded yet)"});
        else
            log_queue_.enqueue({false, "ncs.rt.snac_44kh.decode~: recovered from underrun"});
    }
...
MIN_EXTERNAL(NcsRtSnac_44khDecode);
