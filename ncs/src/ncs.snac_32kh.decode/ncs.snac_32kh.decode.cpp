// -----------------------------------------------------------------------------
// ncs.snac_32kh.decode  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
private:
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac_32kh.decode: ONNX Runtime not available"});
                return;
            }
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.snac_32kh.decode: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.snac_32kh.decode: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
...
MIN_EXTERNAL(NcsSnac_32khDecode);
