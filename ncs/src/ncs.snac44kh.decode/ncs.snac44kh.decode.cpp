// -----------------------------------------------------------------------------
// ncs.snac44kh.decode  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// The model only outputs 44kHz audio; it's resampled to output_sr here
...
// to snac32kh (only sampling rate and weights differ) -- see
// ncs.snac24kh.decode.cpp for the full design rationale this module
// mirrors.
...
// ncs.snac44kh.embedcodes sends its (chunked) embeddings as a `messageix
// <index>` message (index -1 = last chunk) immediately followed by
// `tensor <data...>`, both real Max message selectors -- see
// ncs.snac24kh.encode.cpp's kMaxChunkAtoms comment for the full crash
...
// separately, directly from ncs.snac44kh.encode's second outlet (as
...
// last step in process(), after the 44kHz->output_sr resample -- not
// before it -- so a single final rounding produces the trim count,
// instead of carrying a value pre-rounded in a different (44kHz) domain
// through a second, independent resample.
//
// That's a deliberate, accepted race (see ncs.snac24kh.encode.cpp's
...
class NcsSnac44khDecode : public object<NcsSnac44khDecode>, public CancellableRun
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 44kHz decoder and writes the result to a named buffer~."};
    MIN_TAGS            {"snac, onnx, audio"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> embeddings_in{ this, "(set/load/messageix/tensor) buffer name, model path, or chunked embeddings from ncs.snac44kh.embedcodes" };
    inlet<> length_in{ this, "(float) source duration in seconds -- wire directly from ncs.snac44kh.encode's second outlet" };
    ...
    message<> tensor_msg{this, "tensor", "Embeddings chunk data",
        MIN_FUNCTION {
            try {
                ...
                log_queue_.enqueue({true, "ncs.snac44kh.decode: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};

    // Cold: caches the source duration (seconds) from
    // ncs.snac44kh.encode's second outlet, used (if ever received) by
    // the next tensor completion above. See the class-level comment for
    // the accepted race and the "not connected -> no trim" fallback.
    message<> length_msg{this, "float", "Source duration in seconds, from ncs.snac44kh.encode's second outlet",
        MIN_FUNCTION {
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac44kh.decode: load requires a file path"});
                return {};
            }
...
private:
    ...
    // Resample 44100 -> output_sr FIRST, then trim to trim_samples (an
    ...
    // output_sr-space. Padding (added by ncs.snac44kh.encode before
    ...
    void process(const DecodeJob& job) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac44kh.decode: no model loaded"});
            return;
        }
        auto audio = run_onnx(job.embeddings);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.snac44kh.decode: inference failed"});
            return;
        }
        ...
        if (output_queue_.try_dequeue(audio)) {
            ...
            try {
                if (!ncs_buffer::write_mono(buffer_ref_, audio)) {
                    c74::max::post("%s", "ERROR: ncs.snac44kh.decode: output buffer not found — use 'set <name>' first");
                    return;
                }
                std::string msg = "ncs.snac44kh.decode: wrote " + std::to_string(audio.size())
                                   + " samples to buffer '" + std::string(buffer_ref_.name()) + "'";
                c74::max::post("%s", msg.c_str());
            } catch (const std::exception& ex) {
                c74::max::post("%s", ("ERROR: ncs.snac44kh.decode: write failed — " + std::string(ex.what())).c_str());
            }
        }
    }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac44kh.decode: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.snac44kh.decode: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsSnac44khDecode);
