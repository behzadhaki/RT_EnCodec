// -----------------------------------------------------------------------------
// ncs.rt.snac_48kh.encode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
class NcsRtSnac_48khEncode : public object<NcsRtSnac_48khEncode>
...
public:
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                error("ncs.rt.snac_48kh.encode~: load requires a file path");
                return {};
            }
...
private:
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac_48kh.encode~: ONNX Runtime not available"});
                return;
            }
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.rt.snac_48kh.encode~: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.rt.snac_48kh.encode~: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
...
MIN_EXTERNAL(NcsRtSnac_48khEncode);
