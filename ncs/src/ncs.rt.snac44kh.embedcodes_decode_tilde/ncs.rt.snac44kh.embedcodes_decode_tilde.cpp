// -----------------------------------------------------------------------------
// ncs.rt.snac44kh.embedcodes_decode~  —  fused embedcodes + decode~
// -----------------------------------------------------------------------------
...
// Fuses ncs.rt.snac44kh.embedcodes and ncs.rt.snac44kh.decode~ into ONE
// object running on ONE worker thread. Identical network to snac32kh
// (only sampling rate and weights differ) -- see
// ncs.rt.snac24kh.embedcodes_decode_tilde.cpp for the full fusion
...
class NcsRtSnac44khEmbedcodesDecode : public object<NcsRtSnac44khEmbedcodesDecode>
    , public sample_operator<0, 1>
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 44kHz codebook lookup and decoder continuously from a streamed per-level codes message, fused into one object (no inter-object message relay between embedcodes and decode)."};
    ...
    // This matches ncs.rt.snac44kh.vq (or ncs.rt.snac44kh.encode_vq~),
    // whose outlets fire right-to-left (level3 first, level0 last), so a
    ...
    inlet<> level3_in{ this, "(list) codes... from vq level3 -- cold" };
    ...
    // ncs.rt.snac44kh.encode_vq_tilde.cpp's timing_out comment for why
        ...
        MIN_FUNCTION {
            try {
                ...
                if (inlet == 1) {
                    ...
                    // (e.g. vq's outlet 1-3 never connected here, so its
                    // cache stayed empty) turns into a zero-length
                    ...
                    if (cached_level1_.empty() || cached_level2_.empty() || cached_level3_.empty()) {
                        log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: level1/2/3 codes not yet received "
                              "— connect all four of vq's outlets to inlets 0-3"});
                        return {};
                    }
                    EmbedRequest req;
                    ...
                    // see ncs.rt.snac44kh.vq's class-level comment.
                    input_queue_.enqueue(std::move(req));
                }
            } catch (const std::exception& ex) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: list handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load_embedcodes{this, "load_embedcodes", "Load an ONNX model (.onnx) for the embedcodes stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: load_embedcodes requires a file path"});
                return {};
            }
    ...
    message<> load_decode{this, "load_decode", "Load an ONNX model (.onnx) for the decode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: load_decode requires a file path"});
                return {};
            }
...
private:
    ...
    // -- embedcodes' own compute is tiny compared to decode's, so
    // there's no meaningful throughput benefit to pipelining them across
    ...
    void process(const EmbedRequest& req) {
        ...
        if (!model_loaded_embedcodes_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: no embedcodes model loaded"});
            return;
        }
        auto sum = run_onnx_embedcodes(req);
        if (sum.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: embedcodes inference failed"});
            return;
        }
        int64_t T_block = static_cast<int64_t>(sum.size()) / kLatentDim;
        if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(sum.size())) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: summed embeddings size is not a multiple of 1024 channels"});
            return;
        }
        ...
        if (!model_loaded_decode_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: no decode model loaded"});
            return;
        }
        auto audio = run_onnx_decode(combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: decode inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: unexpected decode output length"});
            return;
        }
    ...
    void load_model_embedcodes(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: ONNX Runtime not available"});
                return;
            }
            ...
            if (input_names_embedcodes_.size() != kNumLevels || output_names_embedcodes_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: failed to load embedcodes model (" + path + ") — "
    ...
    void load_model_decode(const std::string& path) {
        try {
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.embedcodes_decode~: failed to load decode model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    // [1024 x T] -- the only thing decode~ (the last stage here) needs.
...
MIN_EXTERNAL(NcsRtSnac44khEmbedcodesDecode);
