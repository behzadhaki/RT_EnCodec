// -----------------------------------------------------------------------------
// ncs.snac_44kh.vq  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 44kHz quantizer: 4 codebook levels, vq_strides=[8,4,2,1] (coarsest
// first). quantize_encodings.onnx has 1 input ("z") and 4 outputs
// (codes_0/1/2/3), each already at its own native temporal resolution
// (T/8, T/4, T/2, T) -- each level gets its own outlet, so no
// zero-order-hold expansion/interleaving is needed here. Identical
// network to snac_44kh (only sampling rate and weights differ) -- see
// ncs.snac_24kh.vq.cpp for the full design rationale this module mirrors.
static constexpr int kNumLevels = 4;

// ncs.snac_44kh.encode sends chunked embeddings as a `messageix <index>`
...
// it's only ever a pass-through for ncs.snac_44kh.decode's final trim,
// which now gets it directly from ncs.snac_44kh.encode's second outlet).
// Codes (this module's own output) are much smaller than embeddings and
...
struct PendingSend {
    int outlet_id; // 0 = level0, 1 = level1, 2 = level2, 3 = level3
    atoms data;    // codes...
};

class NcsSnac_44khVq : public object<NcsSnac_44khVq>, public CancellableRun
{
public:
    MIN_DESCRIPTION     {"SNAC 44kHz residual vector quantizer: embeddings -> per-level codes."};
    MIN_TAGS            {"snac, onnx"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> control_in{ this, "(messageix/tensor/load) embeddings chunk from ncs.snac_44kh.encode, or model path" };
        ...
        MIN_FUNCTION {
            try {
                ...
                if (pending_is_last_) {
                    ...
                    // worker is still doing -- see ncs.snac_44kh.encode's
                ...
                log_queue_.enqueue({true, "ncs.snac_44kh.vq: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac_44kh.vq: load requires a file path"});
                return {};
            }
...
private:
    ...
    // downstream ncs.snac_44kh.embedcodes (whose leftmost/level0 inlet is
    // hot) sees its cold level1/level2/level3 inlets updated before the
    // trigger.
    void process(const std::vector<float>& z) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac_44kh.vq: no model loaded"});
            return;
        }
        auto result = run_onnx(z);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.snac_44kh.vq: inference failed"});
            return;
        }
    ...
    void flush_output() {
        ...
        if (output_queue_.try_dequeue(item)) {
            switch (item.outlet_id) {
                case 3: level3_out.send(item.data); break;
                case 2: level2_out.send(item.data); break;
                case 1: level1_out.send(item.data); break;
                default: level0_out.send(item.data); break;
            }
        }
    }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac_44kh.vq: ONNX Runtime not available"});
                return;
            }
            ...
            if (output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.snac_44kh.vq: expected " + std::to_string(kNumLevels)
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.snac_44kh.vq: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.snac_44kh.vq: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
