// -----------------------------------------------------------------------------
// ncs.rt.snac32kh.embedcodes  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// ncs.snac32kh.embedcodes (same decode_codes.onnx graph -- the codebook
...
// Identical network to snac44kh (only sampling rate and weights
...
// ncs.rt.snac32kh.decode~'s running context -- every block must be
// looked up in order, none can be dropped. So (like ncs.rt.snac32kh.vq)
...
// ncs.rt.snac32kh.encode_tilde.cpp's kBlockSize(12288)/kModelSampleRate(32000);
...
// ncs.rt.snac32kh.encode_tilde.cpp's kMaxChunkAtoms comment: this is a
...
class NcsRtSnac32khEmbedcodes : public object<NcsRtSnac32khEmbedcodes>
{
public:
    ...
    // This matches ncs.rt.snac32kh.vq, whose outlets fire right-to-left
    // (level3 first, level0 last), so a direct patch cord hookup lands
    // the cold values before the hot trigger arrives.
    inlet<> level0_in{ this, "(list/load) codes... from ncs.rt.snac32kh.vq level0 -- hot, or model path" };
    inlet<> level1_in{ this, "(list) codes... from ncs.rt.snac32kh.vq level1 -- cold" };
    inlet<> level2_in{ this, "(list) codes... from ncs.rt.snac32kh.vq level2 -- cold" };
    inlet<> level3_in{ this, "(list) codes... from ncs.rt.snac32kh.vq level3 -- cold" };

    // Leftmost outlet is always the sum (each level scaled by its
    // level<N>_scale before summing) -- this is the one that normally
    // feeds ncs.rt.snac32kh.decode~; the rest are always the raw,
    ...
    // Parses ncs.rt.snac32kh.vq's format: a bare (headerless) codes list
            ...
            try {
                ...
                if (inlet == 1) {
                    ...
                    if (cached_level1_.empty() || cached_level2_.empty() || cached_level3_.empty()) {
                        log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: level1/level2/level3 codes not yet received "
                              "— connect all four of ncs.rt.snac32kh.vq's outlets to inlets 0-3"});
                        return {};
                    }
                    EmbedRequest req;
                ...
                log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: list handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EmbedRequest& req) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: no model loaded"});
            return;
        }
        auto result = run_onnx(req);
        if (result.empty() || result.size() != kNumLevels + 1) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: ONNX Runtime not available"});
                return;
            }
            // 4 intra-op threads for consistency with encode~/decode~
            // (see ncs.rt.snac32kh.encode_tilde.cpp's comment) -- this
            ...
            if (input_names_.size() != kNumLevels || output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac32kh.embedcodes: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac32khEmbedcodes);
