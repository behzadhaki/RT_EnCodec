// -----------------------------------------------------------------------------
// ncs.rt.snac24kh.decode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 24kHz decoder, real-time variant. Mirrors ncs.rt.snac24kh.encode~'s
...
//    ncs.rt.snac24kh.embedcodes), not a signal -- so the context
...
//    ncs.rt.snac32kh/44kh.decode~, letting modules across all three
...
class NcsRtSnac24khDecode : public object<NcsRtSnac24khDecode>
...
public:
    ...
    inlet<> embeddings_in{ this, "(messageix/tensor/load/reset) quantized embeddings chunk from ncs.rt.snac24kh.embedcodes, channel-major [768 x T], or model path" };
    ...
    // N * ~85ms@24k) for headroom against transient processing
    ...
    // ncs.rt.snac24kh.embedcodes sends its sum_out embeddings as a
    // chunked `messageix <index>` (index -1 = last chunk) immediately
    // followed by `tensor <data...>`, both real Max message selectors --
    // the same protocol ncs.rt.snac32kh/44kh.decode~ use, adopted here
        ...
        MIN_FUNCTION {
            try {
                ...
                if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(new_zq.size())) {
                    log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: embeddings block size is not a multiple of 768 channels"});
                ...
                log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: tensor handling failed — " + std::string(ex.what())});
                accum_embeddings_.clear();
                pending_is_last_ = false;
            }
            return {};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const DecodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: no model loaded"});
            return;
        }
        auto audio = run_onnx(job.combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.decode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac24khDecode);
