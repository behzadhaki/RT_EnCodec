// -----------------------------------------------------------------------------
// ncs.snac_32kh.vq  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// ncs.snac_32kh.encode sends chunked embeddings as a `messageix <index>`
...
// it's only ever a pass-through for ncs.snac_32kh.decode's final trim,
// which now gets it directly from ncs.snac_32kh.encode's second outlet).
...
class NcsSnac_32khVq : public object<NcsSnac_32khVq>, public CancellableRun
{
public:
    MIN_DESCRIPTION     {"SNAC 32kHz residual vector quantizer: embeddings -> per-level codes."};
    MIN_TAGS            {"snac, onnx"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> control_in{ this, "(messageix/tensor/load) embeddings chunk from ncs.snac_32kh.encode, or model path" };
    ...
    message<> tensor_msg{this, "tensor", "Embeddings chunk data",
        MIN_FUNCTION {
            try {
                ...
                log_queue_.enqueue({true, "ncs.snac_32kh.vq: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac_32kh.vq: load requires a file path"});
                return {};
            }
...
private:
    ...
    // downstream ncs.snac_32kh.embedcodes (whose leftmost/level0 inlet is
    // hot) sees its cold level1/level2/level3 inlets updated before the
    // trigger.
    void process(const std::vector<float>& z) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac_32kh.vq: no model loaded"});
            return;
        }
        auto result = run_onnx(z);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.snac_32kh.vq: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac_32kh.vq: ONNX Runtime not available"});
                return;
            }
            ...
            if (output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.snac_32kh.vq: expected " + std::to_string(kNumLevels)
