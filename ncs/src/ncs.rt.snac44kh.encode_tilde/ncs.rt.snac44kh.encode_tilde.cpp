// -----------------------------------------------------------------------------
// ncs.rt.snac44kh.encode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// matching ncs.snac44kh.encode's padding rule) so every block's frame
...
// and ncs.rt.snac44kh.vq reassembles it -- unlike snac24kh.rt's plain
// "list" transport.
//
...
// cost on ncs.rt.snac44kh.embedcodes -> ncs.rt.snac44kh.decode~'s hop
...
// traced to (see ncs.rt.snac44kh.vq's flush_output() comment for the
...
class NcsRtSnac44khEncode : public object<NcsRtSnac44khEncode>
...
public:
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: no model loaded"});
            return;
        }
        auto z = run_onnx(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.encode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac44khEncode);
