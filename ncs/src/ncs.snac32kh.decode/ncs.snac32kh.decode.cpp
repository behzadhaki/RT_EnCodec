// -----------------------------------------------------------------------------
// ncs.snac32kh.decode  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// to snac44kh (only sampling rate and weights differ) -- see
...
// ncs.snac32kh.embedcodes sends its (chunked) embeddings as a `messageix
...
// separately, directly from ncs.snac32kh.encode's second outlet (as
...
class NcsSnac32khDecode : public object<NcsSnac32khDecode>, public CancellableRun
{
public:
    ...
    inlet<> embeddings_in{ this, "(set/load/messageix/tensor) buffer name, model path, or chunked embeddings from ncs.snac32kh.embedcodes" };
    inlet<> length_in{ this, "(float) source duration in seconds -- wire directly from ncs.snac32kh.encode's second outlet" };
    ...
    message<> tensor_msg{this, "tensor", "Embeddings chunk data",
        MIN_FUNCTION {
            try {
                ...
                log_queue_.enqueue({true, "ncs.snac32kh.decode: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};

    // Cold: caches the source duration (seconds) from
    // ncs.snac32kh.encode's second outlet, used (if ever received) by
    // the next tensor completion above. See the class-level comment for
    // the accepted race and the "not connected -> no trim" fallback.
    message<> length_msg{this, "float", "Source duration in seconds, from ncs.snac32kh.encode's second outlet",
        MIN_FUNCTION {
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac32kh.decode: load requires a file path"});
                return {};
            }
...
private:
    ...
    // output_sr-space. Padding (added by ncs.snac32kh.encode before
    ...
    void process(const DecodeJob& job) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac32kh.decode: no model loaded"});
            return;
        }
        auto audio = run_onnx(job.embeddings);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.snac32kh.decode: inference failed"});
            return;
        }
        ...
        if (output_queue_.try_dequeue(audio)) {
            ...
            try {
                if (!ncs_buffer::write_mono(buffer_ref_, audio)) {
                    c74::max::post("%s", "ERROR: ncs.snac32kh.decode: output buffer not found — use 'set <name>' first");
                    return;
                }
                std::string msg = "ncs.snac32kh.decode: wrote " + std::to_string(audio.size())
                                   + " samples to buffer '" + std::string(buffer_ref_.name()) + "'";
                c74::max::post("%s", msg.c_str());
            } catch (const std::exception& ex) {
                c74::max::post("%s", ("ERROR: ncs.snac32kh.decode: write failed — " + std::string(ex.what())).c_str());
            }
        }
    }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac32kh.decode: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.snac32kh.decode: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsSnac32khDecode);
