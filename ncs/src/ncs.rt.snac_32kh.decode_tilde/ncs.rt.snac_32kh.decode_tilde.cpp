// -----------------------------------------------------------------------------
// ncs.rt.snac_32kh.decode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 32kHz decoder, real-time variant. Mirrors ncs.rt.snac_32kh.encode~'s
...
//    ncs.rt.snac_32kh.embedcodes), not a signal -- so the context
...
class NcsRtSnac_32khDecode : public object<NcsRtSnac_32khDecode>
    , public sample_operator<0, 1>
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 32kHz decoder continuously from a streamed quantized-embeddings message (windowed context-trim streaming)."};
    MIN_TAGS            {"snac, onnx, audio"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> embeddings_in{ this, "(messageix/tensor/load/reset) quantized embeddings chunk from ncs.rt.snac_32kh.embedcodes, channel-major [1024 x T], or model path" };
    ...
    // ncs.rt.snac_32kh.embedcodes sends its sum_out embeddings as a
        ...
        MIN_FUNCTION {
            try {
                ...
                if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(new_zq.size())) {
                    log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: embeddings block size is not a multiple of 1024 channels"});
                ...
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: tensor handling failed — " + std::string(ex.what())});
                accum_embeddings_.clear();
                pending_is_last_ = false;
            }
            return {};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const DecodeJob& job) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: no model loaded"});
            return;
        }
        auto audio = run_onnx(job.combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: ONNX Runtime not available"});
                return;
            }
            // 4 intra-op threads -- see ncs.rt.snac_32kh.encode_tilde.cpp's
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.rt.snac_32kh.decode~: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.rt.snac_32kh.decode~: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
...
MIN_EXTERNAL(NcsRtSnac_32khDecode);
