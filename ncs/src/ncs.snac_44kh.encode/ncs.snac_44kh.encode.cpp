// -----------------------------------------------------------------------------
// ncs.snac_44kh.encode  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// sample count) directly to ncs.snac_44kh.decode's second inlet,
// bypassing ncs.snac_44kh.vq/embedcodes entirely -- see
...
class NcsSnac_44khEncode : public object<NcsSnac_44khEncode>, public CancellableRun
{
public:
    ...
    outlet<> length_out{ this, "(float) source duration in seconds -- wire directly to ncs.snac_44kh.decode's second inlet" };
    ...
    message<> bang_msg{this, "bang", "Read the referenced buffer~ and run the encoder",
        MIN_FUNCTION {
            try {
                ...
                if (samples.empty()) {
                    log_queue_.enqueue({true, "ncs.snac_44kh.encode: buffer not found or empty"});
                    return {};
                }
                ...
                if (max_dur_ms > 0 && buffer_sr > 0.0) {
                    ...
                    if (samples.size() > max_dur_samples) {
                        ...
                        log_queue_.enqueue({false, "ncs.snac_44kh.encode: buffer truncated to " + std::to_string(max_dur_ms)
                              + "ms (@max_dur; raise it to encode more)"});
                    }
                }
                if (samples.size() > kMaxInputSamples) {
                    log_queue_.enqueue({true, "ncs.snac_44kh.encode: buffer is " + std::to_string(samples.size())
                ...
                log_queue_.enqueue({true, "ncs.snac_44kh.encode: bang failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac_44kh.encode: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const std::vector<float>& raw_samples, double buffer_sr) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac_44kh.encode: no model loaded"});
            return;
        }
        ...
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.snac_44kh.encode: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac_44kh.encode: ONNX Runtime not available"});
                return;
            }
            ...
            model_loaded_ = true;
            log_queue_.enqueue({false, "ncs.snac_44kh.encode: loaded model (" + path + ")"});
        } catch (const std::exception& ex) {
            log_queue_.enqueue({true, "ncs.snac_44kh.encode: failed to load model — " + std::string(ex.what())});
            model_loaded_ = false;
        }
    }
...
MIN_EXTERNAL(NcsSnac_44khEncode);
