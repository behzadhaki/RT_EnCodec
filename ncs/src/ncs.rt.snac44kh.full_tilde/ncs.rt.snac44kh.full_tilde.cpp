// -----------------------------------------------------------------------------
// ncs.rt.snac44kh.full~  —  fused encode~ + vq + embedcodes + decode~
// -----------------------------------------------------------------------------
...
// Fuses all four standalone SNAC 44kHz rt~ modules (encode~, vq,
...
// snac32kh (only sampling rate and weights differ) -- see
// ncs.rt.snac24kh.full_tilde.cpp for the full fusion rationale; this
// file only documents what's rate-specific.
...
class NcsRtSnac44khFull : public object<NcsRtSnac44khFull>
    , public sample_operator<1, 1>
{
public:
    MIN_DESCRIPTION     {"Runs the full SNAC 44kHz encode -> vq -> embedcodes -> decode pipeline continuously on a signal input, fused into one object (no inter-object message relay between any of the four stages)."};
    ...
    // combined -- see ncs.rt.snac44kh.encode_vq_tilde.cpp's timing_out
    ...
    message<> load_encode{this, "load_encode", "Load an ONNX model (.onnx) for the encode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: load_encode requires a file path"});
                return {};
            }
    ...
    message<> load_vq{this, "load_vq", "Load an ONNX model (.onnx) for the vq stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: load_vq requires a file path"});
                return {};
            }
    ...
    message<> load_embedcodes{this, "load_embedcodes", "Load an ONNX model (.onnx) for the embedcodes stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: load_embedcodes requires a file path"});
                return {};
            }
    ...
    message<> load_decode{this, "load_decode", "Load an ONNX model (.onnx) for the decode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: load_decode requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_encode_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: no encode model loaded"});
            return;
        }
        auto z = run_onnx_encode(job.combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: encode inference failed"});
            return;
        }
        ...
        if (T_total <= T_context) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: encode output shorter than expected context"});
            return;
        }
        ...
        // ---- VQ ----
        if (!model_loaded_vq_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: no vq model loaded"});
            return;
        }
        auto codes = run_onnx_vq(trimmed, T_new);
        if (codes.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: vq inference failed"});
            return;
        }

        // ---- EMBEDCODES ----
        if (!model_loaded_embedcodes_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: no embedcodes model loaded"});
            return;
        }
        EmbedRequest ereq;
        ...
        if (sum.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: embedcodes inference failed"});
            return;
        }
        int64_t T_block = static_cast<int64_t>(sum.size()) / kLatentDim;
        if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(sum.size())) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: summed embeddings size is not a multiple of 1024 channels"});
            return;
        }
        ...
        if (!model_loaded_decode_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: no decode model loaded"});
            return;
        }
        auto audio = run_onnx_decode(combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: decode inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: unexpected decode output length"});
            return;
        }
    ...
    void load_model_encode(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: failed to load encode model (" + path + ") — "
    ...
    void load_model_vq(const std::string& path) {
        try {
            ...
            if (output_names_vq_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: failed to load vq model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    void load_model_embedcodes(const std::string& path) {
        try {
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: failed to load embedcodes model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    void load_model_decode(const std::string& path) {
        try {
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.full~: failed to load decode model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
...
MIN_EXTERNAL(NcsRtSnac44khFull);
