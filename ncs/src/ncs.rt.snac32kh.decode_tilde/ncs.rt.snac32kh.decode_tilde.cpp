// -----------------------------------------------------------------------------
// ncs.rt.snac32kh.decode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 32kHz decoder, real-time variant. Mirrors ncs.rt.snac32kh.encode~'s
...
//    ncs.rt.snac32kh.embedcodes), not a signal -- so the context
...
class NcsRtSnac32khDecode : public object<NcsRtSnac32khDecode>
...
public:
    ...
    inlet<> embeddings_in{ this, "(messageix/tensor/load/reset) quantized embeddings chunk from ncs.rt.snac32kh.embedcodes, channel-major [1024 x T], or model path" };
    ...
    // N * ~384ms@32k) for headroom against transient processing
    ...
    // ncs.rt.snac32kh.embedcodes sends its sum_out embeddings as a
        ...
        MIN_FUNCTION {
            try {
                ...
                if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(new_zq.size())) {
                    log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: embeddings block size is not a multiple of 1024 channels"});
                ...
                log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: tensor handling failed — " + std::string(ex.what())});
                accum_embeddings_.clear();
                pending_is_last_ = false;
            }
            return {};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const DecodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: no model loaded"});
            return;
        }
        auto audio = run_onnx(job.combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: ONNX Runtime not available"});
                return;
            }
            // Adaptive intra-op threads -- see
            // ncs.rt.snac32kh.encode_tilde.cpp's comment: this is the
            ...
            log_queue_.enqueue({true, "ncs.rt.snac32kh.decode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac32khDecode);
