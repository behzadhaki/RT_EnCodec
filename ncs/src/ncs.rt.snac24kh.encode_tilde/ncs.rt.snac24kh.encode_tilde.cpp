// -----------------------------------------------------------------------------
// ncs.rt.snac24kh.encode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// hop_length(512) * vq_strides[0](4), matching ncs.snac24kh.encode's
...
// ncs.snac24kh.encode.cpp, so a single plain "list" message was safe
// here and never needed chunking. data_out nonetheless sends the SAME
// messageix/tensor protocol ncs.rt.snac32kh/44kh.encode~ use (always
// exactly one chunk, since 3072 << kMaxChunkAtoms) purely so
// ncs.rt.snac24kh.vq speaks the identical wire format as its 32k/44k
...
class NcsRtSnac24khEncode : public object<NcsRtSnac24khEncode>
...
public:
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: no model loaded"});
            return;
        }
        auto z = run_onnx(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: unexpected output length"});
            return;
        }
        ...
        // format matches ncs.rt.snac32kh/44kh.encode~'s.
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac24khEncode);
