...
// SNAC 44kHz codebook lookup: 4 levels, vq_strides=[8,4,2,1] (coarsest
// first). decode_codes.onnx has 4 inputs (codes_0/1/2/3, each at its own
// native temporal resolution -- matches ncs.snac_44kh.vq's 4 outlets
...
// summing. Identical network to snac_32kh (only sampling rate and
// weights differ) -- see ncs.snac_24kh.embedcodes.cpp for the full
// design rationale this module mirrors.
static constexpr int kNumLevels = 4;

// Codes in (from ncs.snac_44kh.vq, per level) are a single bare
// (headerless) list -- vq never chunks its output, so no frame/last
// protocol is needed there. Embeddings out (to ncs.snac_44kh.decode and
// friends, per level, chunked) mirror ncs.snac_44kh.encode's data_out
...
// ncs.snac_44kh.decode gets it directly from ncs.snac_44kh.encode's
...
class NcsSnac_44khEmbedcodes : public object<NcsSnac_44khEmbedcodes>, public CancellableRun
{
public:
    MIN_DESCRIPTION     {"SNAC 44kHz codebook lookup: per-level codes -> summed + per-level quantized embeddings."};
    MIN_TAGS            {"snac, onnx"};
    ...
    // This matches ncs.snac_44kh.vq, whose outlets fire right-to-left
    // (level3 first, level0 last), so a direct patch cord hookup lands
    // the cold values before the hot trigger arrives.
    inlet<> level0_in{ this, "(list/load) codes... from ncs.snac_44kh.vq level0 -- hot, or model path" };
    inlet<> level1_in{ this, "(list) codes... from ncs.snac_44kh.vq level1 -- cold" };
    inlet<> level2_in{ this, "(list) codes... from ncs.snac_44kh.vq level2 -- cold" };
    inlet<> level3_in{ this, "(list) codes... from ncs.snac_44kh.vq level3 -- cold" };
    ...
    // stream, same format as ncs.snac_44kh.encode's data_out.
    ...
    // Parses ncs.snac_44kh.vq's format: a bare (headerless) codes list per
            ...
            try {
                ...
                if (inlet == 1) {
                    ...
                    if (cached_level1_.empty() || cached_level2_.empty() || cached_level3_.empty()) {
                        log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: level1/level2/level3 codes not yet received "
                              "— connect all four of ncs.snac_44kh.vq's outlets to inlets 0-3"});
                        return {};
                    }
                    EmbedRequest req;
                    ...
                    // still doing -- see ncs.snac_44kh.encode's bang_msg
                    // comment for why this cancels rather than waits.
                    cancel_active_run();
                    input_queue_.enqueue(std::move(req));
                }
            } catch (const std::exception& ex) {
                log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: list handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EmbedRequest& req) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: no model loaded"});
            return;
        }
        auto result = run_onnx(req);
        if (result.empty() || result.size() != kNumLevels + 1) {
            log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (input_names_.size() != kNumLevels || output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.snac_44kh.embedcodes: expected " + std::to_string(kNumLevels)
