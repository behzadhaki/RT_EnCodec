// -----------------------------------------------------------------------------
// ncs.rt.snac24kh.vq  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// ncs.snac24kh.vq (same quantize_encodings.onnx graph -- quantization is
...
// correct as running it once on a whole buffer~). ncs.rt.snac24kh.encode~
...
// speaks the identical wire format as ncs.rt.snac32kh/44kh.vq, letting
...
// desync ncs.rt.snac24kh.embedcodes/decode~'s context from
// ncs.rt.snac24kh.encode~'s). So the worker loop below deliberately does
...
class NcsRtSnac24khVq : public object<NcsRtSnac24khVq>
{
public:
    MIN_DESCRIPTION     {"SNAC 24kHz residual vector quantizer (real-time): embeddings block -> per-level codes."};
    MIN_TAGS            {"snac, onnx"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> control_in{ this, "(messageix/tensor/load) embeddings chunk from ncs.rt.snac24kh.encode~, or model path" };
    ...
    // ncs.rt.snac24kh.encode~ sends its embeddings block as a chunked
    // `messageix <index>` (index -1 = last chunk) immediately followed
    // by `tensor <data...>`, both real Max message selectors -- the same
    // protocol ncs.rt.snac32kh/44kh.vq use, adopted here purely for
    ...
    message<> tensor_msg{this, "tensor", "Embeddings chunk data",
        MIN_FUNCTION {
            try {
                ...
                log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: load requires a file path"});
                return {};
            }
...
private:
    ...
    // downstream ncs.rt.snac24kh.embedcodes (whose leftmost/level0 inlet
    ...
    void process(const std::vector<float>& z) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: no model loaded"});
            return;
        }
        auto result = run_onnx(z);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: ONNX Runtime not available"});
                return;
            }
            ...
            if (output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.vq: failed to load model (" + path + ") — "
