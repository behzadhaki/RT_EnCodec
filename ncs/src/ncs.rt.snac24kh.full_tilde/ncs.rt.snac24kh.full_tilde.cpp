// -----------------------------------------------------------------------------
// ncs.rt.snac24kh.full~  —  fused encode~ + vq + embedcodes + decode~
// -----------------------------------------------------------------------------
...
// Fuses all four standalone SNAC 24kHz rt~ modules (encode~, vq,
// embedcodes, decode~) into ONE object with ONE worker thread running
// all four stages sequentially per block. Same math and same windowed
// context-trim design as the four separate modules -- see
// ncs.rt.snac24kh.{encode_tilde,vq,embedcodes,decode_tilde}.cpp and
// ncs.rt.snac24kh.{encode_vq_tilde,embedcodes_decode_tilde}.cpp for the
// per-stage rationale already documented there. This file only
// documents what's specific to fusing all four.
static constexpr int64_t kHopLength     = 512;
static constexpr int64_t kBlockFrames   = 4;                          // frames per block (also the encode-side context size, in frames)
static constexpr int64_t kBlockSize     = kHopLength * kBlockFrames;   // 2048 samples, encode-side context size in samples
...
class NcsRtSnac24khFull : public object<NcsRtSnac24khFull>
    , public sample_operator<1, 1>
{
public:
    MIN_DESCRIPTION     {"Runs the full SNAC 24kHz encode -> vq -> embedcodes -> decode pipeline continuously on a signal input, fused into one object (no inter-object message relay between any of the four stages)."};
    ...
    // combined -- see ncs.rt.snac24kh.encode_vq_tilde.cpp's timing_out
    ...
    message<> load_encode{this, "load_encode", "Load an ONNX model (.onnx) for the encode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: load_encode requires a file path"});
                return {};
            }
    ...
    message<> load_vq{this, "load_vq", "Load an ONNX model (.onnx) for the vq stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: load_vq requires a file path"});
                return {};
            }
    ...
    message<> load_embedcodes{this, "load_embedcodes", "Load an ONNX model (.onnx) for the embedcodes stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: load_embedcodes requires a file path"});
                return {};
            }
    ...
    message<> load_decode{this, "load_decode", "Load an ONNX model (.onnx) for the decode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: load_decode requires a file path"});
                return {};
            }
...
private:
    ...
    // per block -- vq and embedcodes are both trivial compute (~1-6ms
    // measured earlier this session) next to encode/decode, so there is
    // no throughput benefit to splitting them across separate threads;
    // doing so would only add cross-thread handoff overhead for no gain.
    void worker_loop() {
        while (!stop_) {
            std::string path;
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_encode_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: no encode model loaded"});
            return;
        }
        auto z = run_onnx_encode(job.combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: encode inference failed"});
            return;
        }
        ...
        if (T_total <= T_context) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: encode output shorter than expected context"});
            return;
        }
        ...
        // ---- VQ ----
        if (!model_loaded_vq_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: no vq model loaded"});
            return;
        }
        auto codes = run_onnx_vq(trimmed, T_new);
        if (codes.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: vq inference failed"});
            return;
        }

        // ---- EMBEDCODES ----
        if (!model_loaded_embedcodes_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: no embedcodes model loaded"});
            return;
        }
        EmbedRequest ereq;
        ...
        if (sum.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: embedcodes inference failed"});
            return;
        }
        int64_t T_block = static_cast<int64_t>(sum.size()) / kLatentDim;
        if (T_block <= 0 || T_block * kLatentDim != static_cast<int64_t>(sum.size())) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: summed embeddings size is not a multiple of 768 channels"});
            return;
        }
        ...
        if (!model_loaded_decode_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: no decode model loaded"});
            return;
        }
        auto audio = run_onnx_decode(combined);
        if (audio.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: decode inference failed"});
            return;
        }
        ...
        if (boundary < 0 || hop <= 0) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: unexpected decode output length"});
            return;
        }
    ...
    void load_model_encode(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: ONNX Runtime not available"});
                return;
            }
            session_options_encode_.SetIntraOpNumThreads(1);
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: failed to load encode model (" + path + ") — "
    ...
    void load_model_vq(const std::string& path) {
        try {
            ...
            session_options_vq_.SetIntraOpNumThreads(1);
            ...
            if (output_names_vq_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: failed to load vq model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    void load_model_embedcodes(const std::string& path) {
        try {
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: failed to load embedcodes model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    void load_model_decode(const std::string& path) {
        try {
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.full~: failed to load decode model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
    ...
    // Returns the scaled sum of all levels, flattened [768 x T] -- the
...
MIN_EXTERNAL(NcsRtSnac24khFull);
