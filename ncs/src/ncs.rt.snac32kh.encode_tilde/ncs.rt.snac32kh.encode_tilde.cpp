// -----------------------------------------------------------------------------
// ncs.rt.snac32kh.encode~  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 32kHz encoder, real-time variant. There is no incremental/cached
...
// (kBlockSize samples @ 32kHz) before the first embeddings can appear.
// Identical network to snac44kh (only sampling rate and weights
// differ) -- see ncs.rt.snac24kh.encode_tilde.cpp for the full design
...
// matching ncs.snac32kh.encode's padding rule) so every block's frame
...
// much bigger block (and latency floor) than snac24kh's 2048 samples --
...
// crash threshold documented in ncs.snac24kh.encode.cpp (~15k-20k atoms
// confirmed to crash Max's own outlet_cache/real_outlet_list via a raw
// memmove overflow). snac24kh's rt~ block (768*4=3072 atoms) never hit
...
// and ncs.rt.snac32kh.vq reassembles it -- unlike snac24kh.rt's plain
// "list" transport.
//
...
// cost on ncs.rt.snac32kh.embedcodes -> ncs.rt.snac32kh.decode~'s hop
// and the model's own ~384ms block-latency floor -- enough added
...
// The model only works at 32kHz; the live host DSP rate is whatever
...
class NcsRtSnac32khEncode : public object<NcsRtSnac32khEncode>
    , public sample_operator<1, 0>
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 32kHz encoder continuously on a live audio signal (windowed context-trim streaming)."};
    ...
    sample operator()(sample in) {
        ...
        while (model_pending_.size() >= static_cast<size_t>(kBlockSize)) {
            ...
            // supersedes-bang queues; see ncs.rt.snac24kh.vq's
            // class-level comment).
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: load requires a file path"});
                return {};
            }
...
private:
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: no model loaded"});
            return;
        }
        auto z = run_onnx(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: unexpected output length"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: ONNX Runtime not available"});
                return;
            }
            // Multiple intra-op threads, not 1: this Run() processes a
            // much bigger window than snac24kh (24576 samples of
            // context+new through an attention-bearing network) inside a
            // real-time budget of only kBlockSize/kModelSampleRate
            // (~384ms @ 32k, ~279ms @ 44k) before decode~'s ring buffer
            // starves and audio drops out. Single-threaded compute was a
            ...
            // regardless, still harmless). adaptive_intra_op_threads()
            ...
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode~: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsRtSnac32khEncode);
