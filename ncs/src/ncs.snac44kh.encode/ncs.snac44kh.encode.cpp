// -----------------------------------------------------------------------------
// ncs.snac44kh.encode  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// SNAC 44kHz: hop_length=384, vq_strides=[8,4,2,1], attn_window_size=32.
// preprocess() in the reference model pads audio to a multiple of
// hop_length * lcm(vq_strides[0], attn_window_size) = 384 * lcm(8,32)
// = 384 * 32 = 12288. Same identical network as 32kHz (only sampling
// rate and weights differ) -- see ncs.snac24kh.encode.cpp for the full
...
// See ncs.snac24kh.encode.cpp for the crash history behind these caps.
static constexpr size_t kMaxChunkAtoms   = 4000;      // per tensor message
static constexpr size_t kMaxInputSamples = 26460000;   // ~10min @ 44kHz

// The model only works at 44kHz; buffer~ content is read at whatever
// sample rate that buffer~ itself is stored at (see buffer_sr below), and
// resampled to 44kHz here before padding/inference -- a one-shot
...
// sample count) directly to ncs.snac44kh.decode's second inlet,
// bypassing ncs.snac44kh.vq/embedcodes entirely -- see
// ncs.snac24kh.encode.cpp's length_out comment for the full rationale
...
class NcsSnac44khEncode : public object<NcsSnac44khEncode>, public CancellableRun
{
public:
    MIN_DESCRIPTION     {"Reads a named buffer~, runs the SNAC 44kHz encoder."};
    ...
    outlet<> length_out{ this, "(float) source duration in seconds -- wire directly to ncs.snac44kh.decode's second inlet" };
        ...
        : output_timer_{this, MIN_FUNCTION { flush_output(); output_timer_.delay(10); return {}; }}
    {
        ...
        // to <package>/models/snac_onnx_exports/44khz/encode_audio_segment.onnx
    ...
    message<> bang_msg{this, "bang", "Read the referenced buffer~ and run the encoder",
        MIN_FUNCTION {
            try {
                ...
                if (samples.empty()) {
                    log_queue_.enqueue({true, "ncs.snac44kh.encode: buffer not found or empty"});
                    return {};
                }
                ...
                if (max_dur_ms > 0 && buffer_sr > 0.0) {
                    ...
                    if (samples.size() > max_dur_samples) {
                        ...
                        log_queue_.enqueue({false, "ncs.snac44kh.encode: buffer truncated to " + std::to_string(max_dur_ms)
                              + "ms (@max_dur; raise it to encode more)"});
                    }
                }
                if (samples.size() > kMaxInputSamples) {
                    log_queue_.enqueue({true, "ncs.snac44kh.encode: buffer is " + std::to_string(samples.size())
                ...
                log_queue_.enqueue({true, "ncs.snac44kh.encode: bang failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.snac44kh.encode: load requires a file path"});
                return {};
            }
...
private:
    ...
    // Resample buffer_sr -> 44kHz, then pad to kPadMultiple for the
    ...
    void process(const std::vector<float>& raw_samples, double buffer_sr) {
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.snac44kh.encode: no model loaded"});
            return;
        }
        ...
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.snac44kh.encode: inference failed"});
            return;
        }
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.snac44kh.encode: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.snac44kh.encode: failed to load model (" + path + ") — "
...
MIN_EXTERNAL(NcsSnac44khEncode);
