// -----------------------------------------------------------------------------
// ncs.rt.snac32kh.encode_vq~  —  fused encode~ + vq
// -----------------------------------------------------------------------------
...
// Fuses ncs.rt.snac32kh.encode~ and ncs.rt.snac32kh.vq into ONE object
// running on ONE worker thread. Identical network to snac24kh (only
// sampling rate, block size, and weights differ) -- see
// ncs.rt.snac24kh.encode_vq_tilde.cpp for the full fusion rationale;
// this file only documents what's rate-specific.
...
class NcsRtSnac32khEncodeVq : public object<NcsRtSnac32khEncodeVq>
    , public sample_operator<1, 0>
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 32kHz encoder and quantizer continuously on a live audio signal, fused into one object (no inter-object message relay between encode and vq)."};
    ...
    sample operator()(sample in) {
        ...
        while (model_pending_.size() >= static_cast<size_t>(kBlockSize)) {
            ...
            // be dropped or collapsed (see ncs.rt.snac32kh.vq's
            // class-level comment).
    ...
    message<> load_encode{this, "load_encode", "Load an ONNX model (.onnx) for the encode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: load_encode requires a file path"});
                return {};
            }
    ...
    message<> load_vq{this, "load_vq", "Load an ONNX model (.onnx) for the vq stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: load_vq requires a file path"});
                return {};
            }
...
private:
    ...
    std::vector<float> model_pending_;  // resampled (32kHz) samples awaiting a full block
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_encode_) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: no encode model loaded"});
            return;
        }
        auto z = run_onnx_encode(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: encode inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: unexpected encode output length"});
            return;
        }
        ...
        if (!model_loaded_vq_) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: no vq model loaded"});
            return;
        }
        auto result = run_onnx_vq(trimmed);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: vq inference failed"});
            return;
        }
    ...
    void load_model_encode(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: ONNX Runtime not available"});
                return;
            }
            ...
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: failed to load encode model (" + path + ") — "
    ...
    void load_model_vq(const std::string& path) {
        try {
            ...
            if (output_names_vq_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac32kh.encode_vq~: failed to load vq model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
...
MIN_EXTERNAL(NcsRtSnac32khEncodeVq);
