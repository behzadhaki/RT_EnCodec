// -----------------------------------------------------------------------------
// ncs.rt.snac44kh.vq  —  generated by Max External Template Generator
// -----------------------------------------------------------------------------
...
// ncs.snac44kh.vq (same quantize_encodings.onnx graph -- quantization is
...
// difference from the buffer module is the transport: ncs.rt.snac44kh.encode~
...
// desync ncs.rt.snac44kh.embedcodes/decode~'s context from
// ncs.rt.snac44kh.encode~'s). So the worker loop below deliberately does
...
class NcsRtSnac44khVq : public object<NcsRtSnac44khVq>
{
public:
    MIN_DESCRIPTION     {"SNAC 44kHz residual vector quantizer (real-time): embeddings block -> per-level codes."};
    MIN_TAGS            {"snac, onnx"};
    MIN_AUTHOR          {"Behzad Haki"};
    inlet<> control_in{ this, "(messageix/tensor/load) embeddings chunk from ncs.rt.snac44kh.encode~, or model path" };
    ...
    // ncs.rt.snac44kh.encode~ sends its embeddings block as a chunked
    ...
    message<> tensor_msg{this, "tensor", "Embeddings chunk data",
        MIN_FUNCTION {
            try {
                ...
                log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: tensor handling failed — " + std::string(ex.what())});
            }
            return {};
        }};
    ...
    message<> load{this, "load", "Load an ONNX model (.onnx)",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: load requires a file path"});
                return {};
            }
...
private:
    ...
    // downstream ncs.rt.snac44kh.embedcodes (whose leftmost/level0 inlet
    ...
    void process(const std::vector<float>& z) {
        ...
        if (!model_loaded_) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: no model loaded"});
            return;
        }
        auto result = run_onnx(z);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: inference failed"});
            return;
        }
    ...
    // ncs.rt.snac44kh.embedcodes, out of a 279ms budget -- draining the
    ...
    void load_model(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: ONNX Runtime not available"});
                return;
            }
            ...
            if (output_names_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac44kh.vq: failed to load model (" + path + ") — "
