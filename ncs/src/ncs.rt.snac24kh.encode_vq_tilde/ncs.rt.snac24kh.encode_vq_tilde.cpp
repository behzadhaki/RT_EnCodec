// -----------------------------------------------------------------------------
// ncs.rt.snac24kh.encode_vq~  —  fused encode~ + vq
// -----------------------------------------------------------------------------
...
// Fuses ncs.rt.snac24kh.encode~ and ncs.rt.snac24kh.vq into ONE object
// running on ONE worker thread. Functionally identical to patching
// encode~'s data_out into vq's control_in -- same two ONNX graphs, same
// windowed context-trim streaming, same math -- but the handoff between
// the two stages is a plain local variable instead of a chunked
// messageix/tensor Max message relayed through flush_output()'s 10ms
// timer. That relay was ~10-40ms per hop after this session's transport
// tuning (see ncs.rt.snac24kh.encode_tilde.cpp's kMaxChunkAtoms/
// kMaxSendsPerTick comments) -- fusing removes it entirely for this hop.
// See ncs.rt.snac24kh.encode_tilde.cpp and ncs.rt.snac24kh.vq.cpp for
// the full per-stage design rationale; this file only documents what's
// different about fusing them.
...
class NcsRtSnac24khEncodeVq : public object<NcsRtSnac24khEncodeVq>
    , public sample_operator<1, 0>
{
public:
    MIN_DESCRIPTION     {"Runs the SNAC 24kHz encoder and quantizer continuously on a live audio signal, fused into one object (no inter-object message relay between encode and vq)."};
    ...
    sample operator()(sample in) {
        ...
        while (model_pending_.size() >= static_cast<size_t>(kBlockSize)) {
            ...
            // be dropped or collapsed (see ncs.rt.snac24kh.vq's
            // class-level comment).
    ...
    message<> load_encode{this, "load_encode", "Load an ONNX model (.onnx) for the encode stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: load_encode requires a file path"});
                return {};
            }
    ...
    message<> load_vq{this, "load_vq", "Load an ONNX model (.onnx) for the vq stage",
        MIN_FUNCTION {
            if (args.size() < 1) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: load_vq requires a file path"});
                return {};
            }
...
private:
    ...
    // Run() then vq Run()) -- vq's own compute is tiny (~1ms, measured
    // earlier this session) compared to encode's, so there's no
    // meaningful throughput benefit to pipelining them across two
    // separate threads, only added complexity.
    void worker_loop() {
        while (!stop_) {
            std::string path;
    ...
    void process(const EncodeJob& job) {
        ...
        if (!model_loaded_encode_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: no encode model loaded"});
            return;
        }
        auto z = run_onnx_encode(combined);
        if (z.empty()) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: encode inference failed"});
            return;
        }
        int64_t T_total = static_cast<int64_t>(z.size()) / kLatentDim;
        if (T_total < kBlockFrames) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: unexpected encode output length"});
            return;
        }
        ...
        if (!model_loaded_vq_) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: no vq model loaded"});
            return;
        }
        auto result = run_onnx_vq(trimmed);
        if (result.size() != kNumLevels) {
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: vq inference failed"});
            return;
        }
    ...
    void load_model_encode(const std::string& path) {
        try {
            ...
            if (!env) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: ONNX Runtime not available"});
                return;
            }
            session_options_encode_.SetIntraOpNumThreads(1);
            session_options_encode_.SetGraphOptimizationLevel(
                GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
            // See ncs.rt.snac24kh.encode_tilde.cpp's load_model() comment
            // for why both Disable* calls are here (disproven heap-
            // corruption theory, kept regardless since harmless).
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: failed to load encode model (" + path + ") — "
    ...
    void load_model_vq(const std::string& path) {
        try {
            ...
            if (output_names_vq_.size() != kNumLevels) {
                log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: expected " + std::to_string(kNumLevels)
            ...
            log_queue_.enqueue({true, "ncs.rt.snac24kh.encode_vq~: failed to load vq model (" + path + ") — "
                                        + std::string(ex.what()) + ". Models are expected at "
...
MIN_EXTERNAL(NcsRtSnac24khEncodeVq);
